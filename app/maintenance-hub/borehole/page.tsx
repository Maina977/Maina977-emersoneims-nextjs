...
export default function BoreholeMaintenanceHub() {
  ...
  const [selectedRepairManual, setSelectedRepairManual] = useState<typeof BOREHOLE_REPAIR_MANUALS[0] | null>(null);
  const [selectedPartsCategory, setSelectedPartsCategory] = useState<string>('submersiblePumps');
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'repair' && (
            <motion.div key="repair" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {BOREHOLE_REPAIR_MANUALS.map(manual => (
                  <div
                    key={manual.id}
          ...
          {activeTab === 'parts' && (
            <motion.div key="parts" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {Object.keys(BOREHOLE_PARTS_CATALOGUE).map(cat => (
                  <button
                    key={cat}
              ...
              <div className="bg-slate-800 rounded-xl overflow-hidden border border-slate-700">
                <table className="w-full">
                  ...
                  <tbody>
                    {(BOREHOLE_PARTS_CATALOGUE[selectedPartsCategory as keyof typeof BOREHOLE_PARTS_CATALOGUE] || []).map((part: { partNumber: string; description: string; brand: string; priceKES: number; application: string }) => (
              ...
              <div className="mt-8">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {BOREHOLE_KENYA_SUPPLIERS.map(supplier => (
          ...
          {activeTab === 'maintenance' && (
            <motion.div key="maintenance" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              {Object.entries(BOREHOLE_MAINTENANCE_SCHEDULES).map(([systemType, schedule]) => (
                <div key={systemType} className="bg-slate-800 rounded-xl p-6 border border-slate-700">
                  <h3 className="text-xl font-bold text-blue-400 mb-4 capitalize">{systemType.replace(/([A-Z])/g, ' $1').trim()}</h3>
                  {Object.entries(schedule).map(([period, tasks]) => (
                    <div key={period} className="mb-4">
                      <h4 className={`font-semibold mb-2 ${
                        period === 'weekly' ? 'text-green-400' :
                        period === 'monthly' ? 'text-yellow-400' :
                        period === 'quarterly' ? 'text-orange-400' :
                        'text-red-400'
