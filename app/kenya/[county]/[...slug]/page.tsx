...
// Curated core services (object form) for in-page links. Every link this
// route emits MUST point at a page in the curated set — see kenyaIndexable.ts.
const CORE_SERVICES: SEOService[] = SEO_SERVICES.filter((s) =>
  CORE_SERVICE_SLUGS.includes(s.slug)
);

// Types for page resolution. Village and village-service pages were
// removed — they were thin, auto-generated doorway pages that Google
// refused to index. Only county-service and constituency(+service)
// pages remain, and only for the curated set in kenyaIndexable.ts.
type PageType = 'county-service' | 'constituency' | 'constituency-service';
...
function resolvePage(
  ...
  // Single segment: county-level service, or a constituency landing page
  ...
  // Two segments: constituency + service
...
// Static params come from the curated indexable registry. Combined with
// `dynamicParams = false` below, ANY /kenya/* URL outside this set returns
// a clean 404 instead of generating a thin doorway page on-demand.
export async function generateStaticParams() {
  return getIndexableKenyaParams();
}
...
function CountyServicePage({
  ...
  // Only link to pages that exist in the curated indexable set.
  const relatedServices = CORE_SERVICES.filter((s) => s.id !== service.id).slice(0, 4);
  // Constituency pages only exist for priority counties — see kenyaIndexable.ts.
  const showConstituencies = isPriorityCounty(county.slug);
  const nearbyConstituencies = showConstituencies
    ? county.constituencies.slice(0, 6)
    : [];
...
// Revalidate the curated static pages once a day.
export const revalidate = 86400;
// dynamicParams = false: any /kenya/* URL not in generateStaticParams (i.e.
// not in the curated indexable set) returns a clean 404 instead of being
// generated on-demand. This is what stops the ~300k thin doorway pages that
// Google was refusing to index.
