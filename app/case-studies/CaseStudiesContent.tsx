...
/**
 * Client-side view for /case-studies.
 *
 * WHY THIS FILE EXISTS (2026-08-01)
 * --------------------------------
 * This code previously lived in page.tsx, which is a SERVER component. It
 * imports `motion` from framer-motion, which needs React context and throws
 * during server rendering. The result: the page rendered NOTHING on the server.
 * A Googlebot fetch of the fresh deployment showed the page's own
 * <main className="min-h-screen bg-black ..."> completely absent — no <h1>, no
 * hero, not even the plain wrapper divs, only the layout's chrome.
 *
 * Client components ARE server-rendered by Next, so moving this code behind a
 * 'use client' boundary puts the whole page back into the initial HTML while
 * keeping every animation working exactly as before.
 *
 * NOTHING WAS DELETED — this is the same code, moved. The page's metadata
 * (including the named client references) stays in page.tsx untouched, because
