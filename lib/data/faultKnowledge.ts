...
export const FAULT_KNOWLEDGE: FaultKnowledge[] = [
  ...
  {
    match: ['synchronisation failure', 'synchronization failure', 'sync failed', 'loss of sync', 'phase match fault', 'sync window fault', 'close angle fault'],
    system: 'Synchronisation',
    severity: 'critical',
    summary: 'The set could not be brought into synchronism with the bus, or lost synchronism after closing. Three conditions must all be satisfied before the breaker may close: voltage magnitude matched, frequency matched with a small positive slip, and phase angle within the permitted window. The check exists because closing outside it applies a violent torque transient to the shaft and a current surge to the windings. Never widen a sync window to force a close.',
    symptoms: ['Breaker fails to close and the sync check remains blocked', 'Repeated close attempts with the set hunting either side of bus frequency', 'Heavy mechanical thump and current surge on closing where the window is too wide', 'Set closing then tripping immediately on reverse power or overcurrent'],
    diagnosis: [
      { step: 1, action: 'Read incoming and running voltage, frequency and phase angle on the controller during a sync attempt', expect: 'Identifies which of the three conditions is not being met, rather than guessing', tools: ['Service tool', 'Controller sync display'] },
      { step: 2, action: 'Compare the set voltage against bus voltage and check the AVR voltage-matching function', expect: 'Voltage matched within the controller window before close is permitted', tools: ['Power quality meter'] },
      { step: 3, action: 'Observe frequency behaviour as the governor attempts to match', expect: 'A small controlled positive slip so the set closes slightly fast and immediately picks up load rather than motoring', tools: ['Power quality meter'] },
      { step: 4, action: 'Verify phase rotation of the incoming set matches the bus', expect: 'Identical rotation — a reversed rotation can never synchronise and must be corrected at the connections', tools: ['Phase rotation meter'] },
      { step: 5, action: 'Check governor and AVR stability settings — hunting prevents the angle window ever being satisfied', expect: 'Stable speed and voltage rather than oscillation around the target', tools: ['Service tool'] },
      { step: 6, action: 'Verify the breaker closing time is correctly configured, since the controller must issue the close command in advance of the angle being reached', expect: 'Configured closing time matching the measured mechanical closing time of the breaker', tools: ['Timing test set'] },
      { step: 7, action: 'Confirm the sync-check relay settings against the commissioning record', expect: 'Settings as commissioned and not widened to work around a fault', tools: ['Protection settings record'] },
    ],
    remedy: [
      'Correct phase rotation at the connections where rotation is reversed — this is a wiring fault, not a settings problem',
      'Re-tune governor and AVR stability so speed and voltage settle rather than hunt',
      'Correct the configured breaker closing time so the close command leads the angle appropriately',
      'Restore sync-check settings to the commissioned values',
      'Repair or replace a breaker whose closing time has drifted with wear',
    ],
    safety: [
      'Never widen the sync window or defeat a sync check to force a close. Out-of-phase closure can shear couplings, damage windings and injure people nearby.',
      'Treat all switchgear as live during synchronisation work and observe the site switching procedure',
      'Ensure the set can be stopped independently before commissioning a sync scheme',
    ],
    tools: ['Power quality meter', 'Phase rotation meter', 'Service tool', 'Timing test set'],
    preventive: ['Record sync settings and breaker closing times at commissioning', 'Verify phase rotation after any switchgear or cabling work', 'Re-test synchronisation annually as part of load-bank testing'],
  },
  {
    match: ['load sharing fault', 'load share fault', 'var share fault', 'kw share fault', 'unequal load sharing', 'droop fault', 'base load fault'],
    system: 'Load Sharing',
    severity: 'critical',
    summary: 'Sets running in parallel are not dividing load in proportion to their ratings. Real power sharing is set by the governors and reactive power sharing by the AVRs, and the two are diagnosed separately. Left uncorrected one set carries more than its share and trips, which transfers the whole load to the remaining sets and can cascade the entire installation.',
    symptoms: ['One set carrying markedly more kW or kVAr than the others', 'A set drifting toward reverse power or motoring', 'Load swinging between sets rather than settling', 'One set tripping on overload while others run lightly loaded'],
    diagnosis: [
      { step: 1, action: 'Record kW and kVAr on every set simultaneously, expressed as a percentage of each set rating', expect: 'Separates a real-power problem from a reactive-power problem, which have different causes', tools: ['Power quality meter'] },
      { step: 2, action: 'Where kW sharing is unequal, compare governor droop settings across all sets', expect: 'Identical droop percentage on every set, or all sets in the same isochronous load-share scheme', tools: ['Service tool'] },
      { step: 3, action: 'Where kVAr sharing is unequal, compare AVR droop or cross-current compensation settings', expect: 'Matched reactive droop across sets; unmatched settings are the usual cause of kVAr circulation', tools: ['Service tool'] },
      { step: 4, action: 'Verify the load-share communication link or analogue share lines between controllers', expect: 'Link healthy and share lines continuous with correct polarity', tools: ['Digital multimeter', 'Service tool'] },
      { step: 5, action: 'Check CT ratios and polarity on every set feeding the sharing scheme', expect: 'Matched ratios and correct polarity — a reversed CT inverts the sharing signal', tools: ['Clamp meter'] },
      { step: 6, action: 'Confirm all sets are on the same operating mode and that none has been left in base-load or fixed-kW mode unintentionally', expect: 'Consistent mode across the scheme', tools: ['Service tool'] },
    ],
    remedy: [
      'Match governor droop across all sets, or commission them into a common isochronous load-share scheme',
      'Match AVR reactive droop or cross-current compensation so kVAr divides correctly',
      'Repair the load-share link and correct share-line polarity',
      'Correct CT ratio and polarity errors',
      'Return a set left in base-load mode to the correct sharing mode',
    ],
    safety: [
      'A set carrying disproportionate load may trip without warning and transfer everything to the remaining plant — plan for that before testing',
      'Reverse power motoring can damage the prime mover; do not leave a set motoring while investigating',
      'Never open-circuit a CT secondary while primary current flows',
    ],
    tools: ['Power quality meter', 'Clamp meter', 'Service tool', 'Digital multimeter'],
    preventive: ['Record droop and share settings for every set in the commissioning pack', 'Verify sharing under real load after any controller replacement', 'Test parallel operation annually rather than assuming it still works'],
  },
  {
    match: ['transfer failure', 'ats failure', 'ats communication error', 'changeover failure', 'retransfer failure', 'transfer switch fault'],
    system: 'Transfer',
    severity: 'critical',
    summary: 'The transfer switch did not move the load as commanded. This is the component that decides whether all the investment in a standby set ever reaches the load, and its failures are frequently silent until an outage. Diagnose the command path, the sensing and the mechanism separately.',
    symptoms: ['Load not transferred despite the set running at rated voltage and frequency', 'Repeated transfer and retransfer cycling', 'Transfer switch stuck in one position', 'Set running unloaded during a genuine mains failure'],
    diagnosis: [
      { step: 1, action: 'Confirm the set actually reached rated voltage and frequency and held them', expect: 'Output within the transfer acceptance window — a set outside the window will correctly be refused', tools: ['Power quality meter'] },
      { step: 2, action: 'Verify the mains-sensing inputs to the transfer controller against measured supply', expect: 'Sensing agreeing with reality; a failed sensing fuse makes a healthy supply look absent or the reverse', tools: ['Digital multimeter'] },
      { step: 3, action: 'Check the four transfer timers against the site requirement: mains-failure delay, warm-up, retransfer delay and cool-down', expect: 'Timers set for the site rather than left at factory defaults', tools: ['Service tool'] },
      { step: 4, action: 'Verify the engine start command wiring from the transfer controller to the genset panel', expect: 'Continuity proven — a broken start signal is a silent failure discovered only during an outage', tools: ['Digital multimeter'] },
      { step: 5, action: 'Operate the switch manually following the site procedure and observe the mechanism', expect: 'Free mechanical movement with no binding, and the interlock preventing both sources closing together', tools: ['Manual operating handle'] },
      { step: 6, action: 'Inspect the main contacts for pitting, welding and heat damage', expect: 'Clean contacts with no signs of welding', tools: ['Inspection light'] },
      { step: 7, action: 'Prove the whole sequence by opening the mains supply deliberately under controlled conditions', expect: 'Full automatic sequence completing within the expected time', tools: ['Service tool'] },
    ],
    remedy: [
      'Replace failed sensing fuses and correct sensing wiring',
      'Set all four timers for the site — a retransfer delay left in seconds causes repeated cycling on an unstable supply',
      'Repair the engine start command circuit',
      'Service or replace a binding mechanism, and replace welded or heavily pitted contacts',
      'Prove the mechanical interlock physically rather than accepting it from the drawing',
      'Exercise the switch monthly so it is not asked to move for the first time during an outage',
    ],
    safety: [
      'A transfer switch has two independent supplies — isolating one does not make it safe. Prove dead on both sides before working.',
      'The mechanical interlock is what prevents paralleling the set onto a live utility supply. Never defeat it.',
      'Manual operation can energise load without warning — confirm downstream is safe before switching',
    ],
    tools: ['Power quality meter', 'Digital multimeter', 'Service tool', 'Inspection light'],
    preventive: ['Exercise the transfer switch monthly under load', 'Record timer settings in the commissioning pack', 'Thermograph the contacts annually at load'],
  },
  {
    match: ['mains failure', 'mains voltage fault', 'mains frequency fault', 'grid fault', 'utility supply fault', 'mains quality fault'],
    system: 'Mains',
    severity: 'warning',
    summary: 'The controller has judged the incoming utility supply to be outside its acceptance window. Frequently the supply really is poor, particularly on weak feeders, and the controller is doing exactly what it should. The engineering question is whether the acceptance window is set appropriately for the supply the site actually has.',
    symptoms: ['Frequent mains-failure starts with no apparent outage', 'Set starting and stopping repeatedly through the day', 'Mains voltage or frequency alarms during known-good supply periods', 'Load cycling between sources'],
    diagnosis: [
      { step: 1, action: 'Log the incoming supply over a full day with a power quality recorder rather than spot-checking it', expect: 'An objective record of voltage, frequency, dips and interruptions', tools: ['Power quality recorder'] },
      { step: 2, action: 'Compare the recorded supply against the controller acceptance window', expect: 'Identifies whether the supply is genuinely out of tolerance or the window is set too tight', tools: ['Service tool'] },
      { step: 3, action: 'Check the mains-sensing fuses and wiring', expect: 'Sensing intact — a blown sensing fuse presents as a permanent mains failure', tools: ['Digital multimeter'] },
      { step: 4, action: 'Verify sensing on all three phases where three-phase sensing is used', expect: 'All phases sensed; a single lost phase makes a healthy supply appear failed', tools: ['Digital multimeter'] },
      { step: 5, action: 'Check the mains-failure detection delay', expect: 'Delay long enough to ride through momentary dips without starting the engine', tools: ['Service tool'] },
    ],
    remedy: [
      'Replace failed sensing fuses and repair sensing wiring',
      'Adjust the acceptance window and detection delay to suit the supply the site actually has, within what the connected load can tolerate',
      'Raise genuinely poor supply quality with the utility, supported by the recorded data',
      'Consider voltage stabilisation where the supply is chronically out of tolerance and the load is sensitive',
    ],
    safety: ['Mains sensing circuits are live at supply potential — isolate and prove dead before working', 'Do not widen the acceptance window beyond what connected equipment can withstand simply to stop nuisance starts'],
    tools: ['Power quality recorder', 'Digital multimeter', 'Service tool'],
    preventive: ['Log supply quality annually so trends are visible', 'Record acceptance settings in the commissioning pack'],
  },
  // ─────────────────────────────────────── GENERATOR-END ELECTRICAL FAULTS ──
  {
    match: ['earth fault', 'ground fault', 'earth leakage', 'stator earth fault', 'insulation fault', 'insulation failure'],
    system: 'Protection',
    severity: 'critical',
    summary: 'Current is returning to earth through a path that is not a designed conductor. On a generator this is both a shock hazard and a fire risk, and it is one of the few faults where re-energising to see whether it recurs is genuinely dangerous. Find the fault before restoring supply.',
    symptoms: ['Earth fault or earth leakage protection operated', 'Nuisance tripping of downstream residual devices', 'Insulation resistance falling on routine testing', 'Damp, tracking or burning smell at machine or cable terminations'],
    diagnosis: [
      { step: 1, action: 'Isolate the affected circuit and prove dead before any testing', expect: 'Circuit isolated, locked off and proven dead', tools: ['Voltage tester', 'Lock-off kit'] },
      { step: 2, action: 'Insulation-test the generator windings to earth with the machine disconnected from the system', expect: 'Insulation resistance above the accepted minimum for the machine voltage, and no downward trend against previous records', tools: ['Insulation tester'] },
      { step: 3, action: 'Insulation-test outgoing cables and the distribution separately from the machine', expect: 'Isolates whether the fault is in the machine or in the installation', tools: ['Insulation tester'] },
      { step: 4, action: 'Where the machine is suspect, measure polarisation index to distinguish moisture from genuine insulation breakdown', expect: 'A polarisation index above the accepted threshold indicates a machine that can be dried rather than rewound', tools: ['Insulation tester with PI function'] },
      { step: 5, action: 'Inspect terminations, cable glands and the terminal box for moisture, dust bridging, vermin damage and tracking', expect: 'Clean, dry terminations with no carbon tracking', tools: ['Inspection light'] },
      { step: 6, action: 'Verify the earthing and neutral arrangement — confirm the neutral-earth bond exists once and only once', expect: 'A single bonding point; a second bond circulates current and produces spurious earth-fault indications', tools: ['Earth tester', 'Clamp meter'] },
      { step: 7, action: 'Measure earth electrode resistance', expect: 'Within the value specified for the installation class', tools: ['Earth resistance tester'] },
    ],
    remedy: [
      'Repair or replace the faulty cable, termination or winding identified by testing',
      'Dry a machine whose polarisation index shows moisture rather than breakdown, then re-test before returning it to service',
      'Remove a duplicate neutral-earth bond',
      'Improve the earth electrode where resistance is above the required value',
      'Clean and re-seal terminal boxes and restore ingress protection',
    ],
    safety: [
      'Do not re-energise a circuit with a known earth fault to "see if it holds" — this risks fire and electrocution',
      'Insulation testing applies high voltage: isolate, warn others, and discharge the circuit afterwards',
      'Treat generator terminals as live until proven dead; a generator can be back-energised from the load side',
    ],
    tools: ['Insulation tester', 'Earth resistance tester', 'Clamp meter', 'Voltage tester'],
    preventive: ['Insulation-test and record results annually so trends are visible before failure', 'Maintain terminal box sealing against moisture and vermin', 'Verify the single-bond arrangement after any electrical alteration'],
  },
  {
    match: ['loss of field', 'loss of excitation', 'excitation fault', 'exciter failure', 'field failure', 'overexcitation', 'underexcited'],
    system: 'Electrical',
    severity: 'critical',
    summary: 'The excitation system is not maintaining the generator field correctly. Losing excitation causes the machine to lose its ability to produce reactive power, draw magnetising current from the bus and overheat the rotor. Over-excitation drives the opposite problem and overheats the field. Either way the machine cannot regulate voltage.',
    symptoms: ['Loss of field or excitation protection operated', 'Output voltage collapsing or unable to be raised', 'Reactive power drawn from the bus instead of supplied', 'Rotor or field temperature rising abnormally'],
    diagnosis: [
      { step: 1, action: 'Measure generator output voltage and field voltage together during a controlled run', expect: 'Field responding to AVR command and output following it', tools: ['Digital multimeter', 'Power quality meter'] },
      { step: 2, action: 'Verify the AVR sensing input is present and correct', expect: 'Sensing present — an AVR that cannot see output voltage will drive the field to a limit', tools: ['Digital multimeter'] },
      { step: 3, action: 'Check AVR sensing fuses', expect: 'Intact; a blown sensing fuse is a very common cause of excitation faults', tools: ['Digital multimeter'] },
      { step: 4, action: 'Test the rotating rectifier diodes where the machine is brushless', expect: 'All diodes healthy — a failed diode produces a characteristic unstable or reduced output', tools: ['Digital multimeter'] },
      { step: 5, action: 'Measure exciter field and main field winding resistance', expect: 'Within specification and balanced', tools: ['Digital multimeter', 'Micro-ohmmeter'] },
      { step: 6, action: 'Inspect brushes and slip rings where the machine is brushed', expect: 'Adequate brush length, correct spring pressure, clean rings with even wear', tools: ['Inspection light'] },
      { step: 7, action: 'Where residual magnetism has been lost, attempt a controlled field flash per the machine instructions', expect: 'Output builds and holds', tools: ['Field flashing equipment'] },
    ],
    remedy: [
      'Replace failed AVR sensing fuses and repair sensing wiring',
      'Replace failed rotating rectifier diodes',
      'Replace worn brushes and service slip rings',
      'Field-flash a machine that has lost residual magnetism',
      'Replace the AVR once sensing, field circuit and rectifier test sound',
      'Refer the machine for rewind where field winding resistance is out of specification',
    ],
    safety: [
      'Field circuits carry dangerous voltages and stored energy — isolate and discharge before working',
      'Rotating rectifiers are inaccessible while running; never attempt inspection on a turning machine',
      'Generator terminals may be live even at low excitation — prove dead before contact',
    ],
    tools: ['Digital multimeter', 'Power quality meter', 'Insulation tester', 'Micro-ohmmeter'],
    preventive: ['Check AVR sensing fuses at every service', 'Inspect brushes and slip rings on interval for brushed machines', 'Record field resistance values so drift is visible'],
  },
  {
    match: ['power factor fault', 'pf control fault', 'var control fault', 'reactive power fault', 'pf correction fault'],
    system: 'Electrical',
    severity: 'warning',
    summary: 'Reactive power or power factor is outside the intended band. On a generator this matters because reactive current heats the windings without producing useful output, and a machine at poor power factor reaches its thermal limit well below its kW rating. Establish whether the cause is the load, the excitation control or a correction system misbehaving.',
    symptoms: ['Power factor outside the target band', 'Machine reaching current limit below rated kW', 'Reactive power flowing in an unexpected direction', 'Capacitor bank stages switching repeatedly'],
    diagnosis: [
      { step: 1, action: 'Record kW, kVAr and power factor at the machine terminals over a representative load period', expect: 'A clear picture of whether the load or the machine is responsible', tools: ['Power quality recorder'] },
      { step: 2, action: 'Characterise the connected load — motors, drives and lightly loaded transformers draw magnetising current', expect: 'Load profile explaining the reactive demand', tools: ['Power quality recorder'] },
      { step: 3, action: 'Where power factor correction is fitted, verify stage switching and check for failed capacitors', expect: 'Stages switching correctly with healthy capacitors and intact discharge resistors', tools: ['Capacitance meter', 'Clamp meter'] },
      { step: 4, action: 'Measure voltage distortion before relying on correction equipment', expect: 'Distortion low enough that untuned capacitors will not resonate — this is where detuned reactors become necessary', tools: ['Power quality analyser'] },
      { step: 5, action: 'Check AVR reactive droop settings when running in parallel', expect: 'Settings matched across sets so reactive current does not circulate between machines', tools: ['Service tool'] },
    ],
    remedy: [
      'Correct AVR reactive droop settings so machines share reactive load properly',
      'Replace failed capacitors and repair stage switching',
      'Fit detuned reactors where harmonic distortion makes plain capacitors unsafe',
      'Address the load itself — soft starters, drives and correctly sized transformers reduce reactive demand at source',
    ],
    safety: [
      'Power factor correction capacitors store dangerous energy after isolation. Wait for the discharge period and prove dead before touching terminals.',
      'Never connect plain capacitors to a harmonic-rich supply — resonance can destroy the bank violently',
    ],
    tools: ['Power quality analyser', 'Capacitance meter', 'Clamp meter', 'Service tool'],
    preventive: ['Log power factor as part of annual power quality checks', 'Inspect capacitor banks for bulging and leakage annually', 'Re-assess correction sizing whenever significant load is added'],
  },
  {
    match: ['harmonic distortion', 'thd fault', 'power quality fault', 'voltage distortion', 'current distortion'],
    system: 'Electrical',
    severity: 'warning',
    summary: 'Harmonic distortion is above the acceptable level. Non-linear loads — variable speed drives, UPS rectifiers, LED drivers and switch-mode supplies — draw current in pulses rather than sinusoids. On a generator the effect is worse than on a utility supply because the machine has higher source impedance, so the distorted current produces distorted voltage that then affects everything else on the bus.',
    symptoms: ['Voltage or current distortion above the limit', 'Neutral conductor running hotter than the phases', 'Nuisance tripping of protective devices with no overload present', 'Transformer or alternator running hot at less than rated load'],
    diagnosis: [
      { step: 1, action: 'Measure voltage and current distortion separately at the generator terminals and at the main board', expect: 'Distinguishes distortion generated by the load from distortion appearing on the supply', tools: ['Power quality analyser'] },
      { step: 2, action: 'Identify the harmonic orders present', expect: 'The order pattern identifies the source type — six-pulse rectifiers and single-phase switch-mode loads produce different signatures', tools: ['Power quality analyser'] },
      { step: 3, action: 'Measure neutral current and compare against phase currents', expect: 'Neutral current above the highest phase current confirms triplen harmonics from single-phase non-linear load', tools: ['Clamp meter'] },
      { step: 4, action: 'Assess generator loading and the ratio of non-linear to linear load', expect: 'A judgement on whether the machine is adequately rated for the harmonic content it is being asked to supply', tools: ['Power quality analyser'] },
      { step: 5, action: 'Thermograph the neutral bar, terminations and the alternator at load', expect: 'No hot spots; neutral terminations run cool on a healthy installation', tools: ['Thermal imaging camera'] },
    ],
    remedy: [
      'Uprate the neutral conductor where triplen harmonics are confirmed',
      'Fit harmonic filtering or specify low-distortion input stages on large rectifier loads such as UPS systems',
      'Separate sensitive loads from the main harmonic sources where practical',
      'Specify an alternator with appropriate derating for the harmonic content, rather than assuming nameplate kVA is available',
      'Fit detuned reactors on any power factor correction present',
    ],
    safety: ['A neutral conductor has no overcurrent device protecting it — harmonic overheating is invisible to the breaker and is a genuine fire risk', 'Thermal imaging must be done at load with covers removed by a competent person under a live working procedure'],
    tools: ['Power quality analyser', 'Clamp meter with true RMS', 'Thermal imaging camera'],
    preventive: ['Measure distortion at commissioning and record it as a baseline', 'Reassess whenever significant electronic load is added', 'Include neutral current in routine measurements, not just phase currents'],
  },
  {
    match: ['winding temperature high', 'stator temperature high', 'alternator overheating', 'winding overtemperature'],
    system: 'Electrical',
    severity: 'critical',
    summary: 'Generator winding temperature is above its limit. Insulation life falls sharply with sustained over-temperature, so a machine repeatedly run hot will fail early even if it never trips. Establish whether the cause is electrical loading, cooling, or a measurement problem before accepting a derate.',
    symptoms: ['Winding temperature alarm or trip', 'Machine hot to the touch at the frame beyond normal', 'Temperature rising faster than load would explain', 'Repeated trips on hot days but not cool ones'],
    diagnosis: [
      { step: 1, action: 'Record load current per phase against the machine rating and check balance', expect: 'Load within rating and balanced; unbalanced loading heats one phase disproportionately', tools: ['Clamp meter', 'Power quality meter'] },
      { step: 2, action: 'Measure power factor and reactive loading', expect: 'A machine at poor power factor carries high current for modest kW and heats accordingly', tools: ['Power quality analyser'] },
      { step: 3, action: 'Check harmonic content, which heats windings without contributing output', expect: 'Distortion within the level the machine is rated for', tools: ['Power quality analyser'] },
      { step: 4, action: 'Inspect the cooling air path — inlet filters, screens, air passages and any recirculation of hot air', expect: 'Clear, unobstructed cooling airflow at the design temperature', tools: ['Inspection light', 'Anemometer'] },
      { step: 5, action: 'Compare the embedded temperature detector reading against a surface measurement', expect: 'Consistent readings; a drifted detector produces spurious trips', tools: ['Thermal imaging camera', 'Digital multimeter'] },
      { step: 6, action: 'Measure ambient temperature at the machine intake and compare against the rating basis', expect: 'Ambient within the design assumption, or a derate correctly applied', tools: ['Thermometer'] },
    ],
    remedy: [
      'Reduce or rebalance load so no phase exceeds its share',
      'Correct power factor and harmonic loading at source',
      'Clear obstructed cooling passages and stop hot-air recirculation',
      'Improve room ventilation or apply the correct derate for the real ambient',
      'Replace a drifted temperature detector once independent measurement disproves the reading',
    ],
    safety: ['Allow the machine to cool before opening enclosures', 'Do not defeat a winding temperature trip to keep load on — the protection is what preserves the insulation'],
    tools: ['Power quality analyser', 'Clamp meter', 'Thermal imaging camera', 'Anemometer'],
    preventive: ['Thermograph the machine annually at load', 'Keep cooling intakes and screens clean', 'Balance single-phase loads across the three phases'],
  },
  // ────────────────────────────────────────── MECHANICAL AND PREDICTIVE ──
  {
    match: ['vibration high', 'excessive vibration', 'bearing fault', 'bearing failure', 'bearing condition', 'vibration analysis alert'],
    system: 'Mechanical',
    severity: 'critical',
    summary: 'Vibration is above the acceptable level for the machine. Vibration is the earliest reliable warning of mechanical deterioration, and the frequency content identifies the cause far more precisely than the overall level does. Rising vibration on a generating set is usually alignment, balance, bearing condition or a loosening foundation.',
    symptoms: ['Vibration alarm or measurably rising trend', 'Audible change in running noise', 'Bearing temperature rising alongside vibration', 'Fasteners loosening or coupling elements wearing rapidly'],
    diagnosis: [
      { step: 1, action: 'Measure overall vibration at the bearing housings in all three axes and compare against the ISO evaluation zones for the machine class', expect: 'An objective severity assessment rather than a subjective judgement', tools: ['Vibration analyser'] },
      { step: 2, action: 'Capture a frequency spectrum rather than relying on the overall level', expect: 'The dominant frequency identifies the cause — running speed indicates unbalance, twice running speed indicates misalignment, and bearing defect frequencies indicate bearing damage', tools: ['Vibration analyser'] },
      { step: 3, action: 'Check alignment between engine and alternator where they are separately coupled', expect: 'Alignment within the coupling manufacturer tolerance, checked hot as well as cold', tools: ['Laser alignment kit'] },
      { step: 4, action: 'Inspect anti-vibration mounts for collapse, oil contamination and uneven compression', expect: 'Mounts evenly loaded and resilient; collapsed mounts transmit vibration into the base', tools: ['Inspection light'] },
      { step: 5, action: 'Check holding-down bolts and baseframe fasteners for tightness and the baseframe for cracking', expect: 'All fasteners to torque and no cracking at welds', tools: ['Torque wrench'] },
      { step: 6, action: 'Measure bearing temperature alongside vibration', expect: 'Temperature within normal range; rising temperature with bearing frequencies confirms bearing deterioration', tools: ['Thermal imaging camera'] },
      { step: 7, action: 'Inspect the coupling element for wear and the fan for damage or debris', expect: 'Coupling in good condition and fan undamaged and clean', tools: ['Inspection light'] },
    ],
    remedy: [
      'Re-align the machine to the coupling tolerance and re-check hot',
      'Replace collapsed anti-vibration mounts as a complete set rather than individually',
      'Re-torque holding-down bolts and repair baseframe cracking properly rather than re-welding repeatedly',
      'Replace bearings showing defect frequencies before they fail, and replace in pairs',
      'Balance the rotor where unbalance is confirmed by spectrum analysis',
      'Replace worn coupling elements and clean or replace a damaged fan',
    ],
    safety: [
      'A machine with high vibration can fail suddenly — do not stand in line with the coupling or fan while it runs',
      'Vibration measurement on a running machine requires guards in place and a safe access position',
      'Stop the set immediately if vibration rises sharply during operation',
    ],
    tools: ['Vibration analyser', 'Laser alignment kit', 'Torque wrench', 'Thermal imaging camera'],
    preventive: ['Record a vibration baseline at commissioning and trend against it', 'Check alignment after any engine or alternator disturbance', 'Inspect anti-vibration mounts annually', 'Replace bearings on condition rather than on failure'],
  },
  {
    match: ['emergency stop active', 'emergency stop', 'safety circuit fault', 'trip circuit fault', 'lockout active'],
    system: 'Protection',
    severity: 'shutdown',
    summary: 'An emergency stop or a latched protective lockout is holding the set inoperable. The circuit did its job. The task is to establish which device operated and why — clearing a lockout without knowing what caused it discards the only evidence of a real fault and risks restarting into it.',
    symptoms: ['Set will not start and reports emergency stop or lockout', 'Emergency stop indication on the panel', 'Set stopped abruptly during operation', 'Reset attempts failing to clear the condition'],
    diagnosis: [
      { step: 1, action: 'Read and record the shutdown history before clearing anything', expect: 'The originating alarm identified — this is the evidence and clearing destroys it', tools: ['Service tool', 'Controller event log'] },
      { step: 2, action: 'Physically check every emergency stop station, including remote ones outside the genset room', expect: 'All stations released; a remote station operated by someone else is a common and easily missed cause', tools: ['Inspection light'] },
      { step: 3, action: 'Verify the continuity of the emergency stop loop, which must be a normally-closed circuit', expect: 'Continuity through the whole loop with all stations released', tools: ['Digital multimeter'] },
      { step: 4, action: 'Confirm no other protective device is wired into the same loop and latched', expect: 'The loop containing only genuine emergency stop devices, not mixed with other trips', tools: ['Wiring diagram'] },
      { step: 5, action: 'Inspect the loop wiring for damage, moisture and corroded terminals', expect: 'Sound wiring; an open circuit anywhere presents identically to a pressed button', tools: ['Inspection light'] },
      { step: 6, action: 'Test the emergency stop function deliberately after repair', expect: 'The set stops immediately when any station is operated', tools: [] },
    ],
    remedy: [
      'Release the operated station once it is confirmed safe to do so and the cause is understood',
      'Repair open circuits, corroded terminals and damaged wiring in the loop',
      'Separate other protective trips from the emergency stop loop where they have been wired in together',
      'Replace a faulty emergency stop switch',
      'Resolve the underlying fault first where the lockout followed a genuine protective shutdown',
    ],
    safety: [
      'Never bypass or link out an emergency stop circuit to get a set running. It is the last line of protection for the people around it.',
      'Confirm why the set was stopped before restarting — someone may have stopped it deliberately and be working on it',
      'Prove the emergency stop function works before returning the set to service',
    ],
    tools: ['Service tool', 'Digital multimeter', 'Wiring diagram'],
    preventive: ['Function-test every emergency stop station at each service', 'Keep emergency stop loops free of other trip devices', 'Maintain an accurate record of every station location'],
  },
  {
    match: ['service due', 'maintenance due', 'service interval', 'runtime service', 'oil life expired', 'filter life expired', 'maintenance reminder'],
    system: 'Maintenance',
    severity: 'info',
    summary: 'A scheduled maintenance interval has been reached. This is not a fault — it is the control system doing what it was configured to do. It matters because deferred servicing is the origin of most of the genuine faults elsewhere in this database, and because a counter reset without the work being done makes the whole schedule meaningless.',
    symptoms: ['Service due indication on the panel', 'Running hours or calendar interval reached', 'Consumable life counter expired', 'Indication returning shortly after a reset'],
    diagnosis: [
      { step: 1, action: 'Read the running hours, the interval setting and the date of the last recorded service', expect: 'Confirms which interval has been reached and whether the record is accurate', tools: ['Service tool', 'Maintenance log'] },
      { step: 2, action: 'Establish which specific service is due — intervals differ for oil, fuel, air, coolant and major inspections', expect: 'The correct scope identified rather than assuming an oil change', tools: ['Maintenance schedule'] },
      { step: 3, action: 'Check whether the interval setting matches the manufacturer schedule and the site duty', expect: 'Intervals appropriate for the duty; dusty or high-load sites need shorter intervals than the default', tools: ['Service tool'] },
      { step: 4, action: 'Review the condition-based indicators alongside the counter — oil analysis, filter differential pressure and coolant condition', expect: 'Condition data supporting or challenging the calendar-based interval', tools: ['Oil sample kit', 'Differential gauge'] },
    ],
    remedy: [
      'Carry out the scheduled work, then reset the counter — never reset the counter first',
      'Adjust interval settings to suit the real site duty rather than leaving factory defaults on a harsh site',
      'Record the work and the hours in the maintenance log so the next interval is calculated correctly',
      'Use condition data to move toward condition-based servicing where the plant justifies it',
    ],
    safety: ['Isolate the set and prevent automatic starting before any maintenance work', 'On standby plant, confirm with the site that the set may be taken out of service before starting work'],
    tools: ['Service tool', 'Oil sample kit', 'Maintenance log'],
    preventive: ['Keep an accurate maintenance record per set', 'Set intervals for the actual duty and environment', 'Trend oil analysis rather than relying on hours alone'],
  },
  {
    match: ['digital input fault', 'digital output fault', 'analog output fault', 'binary input fault', 'binary output fault', 'relay output fault', 'io fault', 'i/o fault', 'driver fault', 'harness fault', 'open circuit fault'],
    system: 'Control',
    severity: 'warning',
    summary: 'A controller input or output circuit is not behaving as configured. Because these circuits connect the controller to everything around it, a single faulty channel can present as an apparently unrelated plant fault. Establish whether the problem is the field device, the wiring or the controller channel before replacing anything.',
    symptoms: ['Input or output channel fault reported', 'A function not operating despite the controller commanding it', 'An input stuck in one state regardless of the field device', 'Intermittent behaviour that follows vibration or temperature'],
    diagnosis: [
      { step: 1, action: 'Identify the specific channel and what it is connected to from the wiring schedule', expect: 'The field device and its function identified before any testing', tools: ['Wiring diagram'] },
      { step: 2, action: 'For an input, operate the field device and watch the controller live status', expect: 'Status following the device; no change indicates wiring or channel, not the device', tools: ['Service tool'] },
      { step: 3, action: 'Measure the voltage at the controller terminal in both device states', expect: 'A clear change between states; no change locates the fault upstream of the controller', tools: ['Digital multimeter'] },
      { step: 4, action: 'For an output, command it manually with the service tool and measure at the terminal', expect: 'The terminal switching as commanded; no switching indicates the channel, switching without effect indicates the load or its wiring', tools: ['Service tool', 'Digital multimeter'] },
      { step: 5, action: 'Check the driven load for a short or open circuit, and confirm it is within the channel rating', expect: 'Load within rating and not shorted — an overloaded output channel is a common cause of driver failure', tools: ['Digital multimeter'] },
      { step: 6, action: 'Inspect terminals and the harness for corrosion, loose conductors and chafing', expect: 'Clean tight terminations and undamaged insulation', tools: ['Inspection light'] },
      { step: 7, action: 'Substitute a spare channel where available to confirm the diagnosis before replacing the controller', expect: 'Correct behaviour on the spare channel confirming a failed channel rather than a system fault', tools: ['Service tool'] },
    ],
    remedy: [
      'Repair wiring faults and re-terminate corroded or loose connections',
      'Replace a failed field device once wiring is proven',
      'Correct an overloaded output by fitting an interposing relay rather than driving the load directly',
      'Move the function to a spare channel where a single channel has failed',
      'Replace the controller or its I/O module only once the channel is proven faulty and no spare exists',
    ],
    safety: ['Isolate before disconnecting controller terminals', 'Forcing outputs during testing can operate plant unexpectedly — confirm the area is clear and the plant is safe to operate before commanding any output'],
    tools: ['Service tool', 'Digital multimeter', 'Wiring diagram', 'Inspection light'],
    preventive: ['Keep an accurate wiring schedule with the panel', 'Use interposing relays for anything near a channel rating', 'Protect harnesses at every pass-through and clamp'],
  },
  {
...
/**
 * SUBSYSTEM MAPPING FOR TEMPLATE / RANGE-BASED ENTRIES
 * ====================================================
 * The controller template entries in lib/generator-oracle/controllerFaultCodes.ts
 * do not have a verified individual meaning, but they DO carry a real subsystem
 * classification — a code sitting in a controller's coolant range is a coolant
 * code even when we have not confirmed exactly which coolant fault it is.
 *
 * That makes subsystem-level diagnostic content legitimate and useful for them:
 * "we have not verified what this specific number means on your model, but it is
 * in the coolant range, and this is how a coolant fault is diagnosed on this
 * kind of plant." It gives a technician a real place to start instead of a dead
 * end, without ever claiming a specific meaning we have not checked.
 *
 * Order matters — the first keyword that matches wins, so the more specific
 * terms are listed before the general ones.
 */
const SUBSYSTEM_ROUTES: Array<[string[], string]> = [
  // Synchronisation and paralleling — listed first because these terms are
  // specific and would otherwise be caught by the broader electrical routes.
  [['sync', 'phase match', 'phase angle', 'close angle', 'slip', 'dead bus', 'first on', 'f match', 'v match', 'frequency match', 'voltage match', 'frequency diff', 'voltage diff', 'f bias', 'v bias', 'parallel', 'grid parallel', 'mains parallel', 'multi-set', 'msc'], 'synchronisation failure'],
  [['load share', 'load sharing', 'kw share', 'var share', 'var comp', 'droop', 'base load', 'load balance', 'load priority', 'load ramp', 'load step', 'load acceptance', 'load rejection', 'load factor', 'load limit', 'load profile', 'peak load', 'peak shave', 'peak shaving', 'demand', 'import', 'export', 'kw', 'kvar', 'kva'], 'load sharing fault'],
  // Transfer and utility supply
  [['transfer', 'ats', 'changeover', 'retransfer', 'backfeed', 'interlocking', 'isolator', 'bypass'], 'transfer failure'],
  [['mains', 'grid', 'utility freq', 'anti-island', 'island', 'rocof', 'g59', 'g99', 'ieee 1547', 'grid code', 'grid tie', 'grid form', 'microgrid', 'net meter', 'smart grid', 'demand response', 'df/dt'], 'mains failure'],
  // Generator-end protection and excitation
  [['earth fault', 'ground fault', 'earth leakage', 'stator earth', 'insulation', 'residual', 'ground'], 'earth fault'],
  [['loss of field', 'field fail', 'excitation', 'exciter', 'overexcitation', 'underexcited', 'field'], 'loss of field'],
  [['power factor', 'pf control', 'pf correction', 'capacitor'], 'power factor fault'],
  [['harmonic', 'thd', 'distortion', 'power quality', 'surge', 'swell', 'noise trend'], 'harmonic distortion'],
  [['winding temperature', 'winding', 'stator', 'rotor'], 'winding temperature high'],
  // Mechanical and predictive
  [['vibration', 'bearing', 'wear analysis', 'acoustic analysis', 'thermal analysis', 'gasket condition', 'seal condition', 'belt wear', 'hose condition', 'pump wear', 'efficiency trend', 'engine trend', 'power trend', 'emission trend'], 'vibration high'],
  // Safety chain
  [['emergency stop', 'emergency', 'safety', 'trip circuit', 'lockout'], 'emergency stop active'],
  // Service scheduling
  [['maintenance', 'service', 'runtime', 'cycles', 'hours service', 'oil life', 'filter life', 'coolant life', 'reminder', 'schedule', 'wear counter', 'counter', 'calendar'], 'service due'],
  // Controller I/O and wiring
  [['binary input', 'binary output', 'digital input', 'digital output', 'analog output', 'relay output', 'input/output', 'driver fault', 'harness fault', 'open circuit', 'connector', 'shield ground', 'relay', 'contactor', 'switch', 'i/o'], 'digital input fault'],
  // Lubrication
  [['oil pressure', 'oil shutdown', 'gallery pressure', 'oil flow'], 'oil pressure low'],
  [['oil temperature'], 'oil temperature high'],
  [['oil level', 'oil filter', 'oil quality', 'oil life', 'oil analysis', 'oil cooler', 'lubrication', 'lube service', 'oil'], 'oil pressure low'],
  // Cooling
  [['coolant level', 'expansion tank', 'coolant additive'], 'coolant level low'],
  [['fan clutch', 'radiator shutters', 'ventilation', 'fan'], 'engine cooling fan failure'],
  [['coolant temp', 'coolant', 'radiator', 'thermostat', 'water pump', 'heat exchanger', 'overheat', 'overtemperature', 'temp shutdown', 'intercooler', 'charge air'], 'coolant temperature high'],
  // Fuel
  [['rail pressure', 'rail volume'], 'low fuel rail pressure'],
  [['injector', 'injection', 'nozzle', 'spray pattern'], 'fuel injector fault'],
  [['fuel temp'], 'fuel temperature high'],
  [['fuel level', 'level sensor'], 'fuel level sensor failure'],
  [['fuel pump'], 'fuel pump relay failure'],
  [['solenoid', 'fuel control', 'relief valve', 'bypass valve'], 'fuel solenoid valve failure'],
  [['fuel filter', 'fuel pressure', 'fuel quality', 'fuel rate', 'fuel service', 'dilution', 'contamination', 'fuel'], 'low fuel rail pressure'],
  // Air and boost
  [['air filter', 'restriction'], 'air filter restriction'],
  [['turbo', 'wastegate', 'vgt', 'boost'], 'turbo boost low'],
  [['manifold pressure', 'pressure sensor', 'transducer', 'transmitter', 'sensor drift', 'sensor range', 'sensor wire', 'rationality', 'probe', 'sensor'], 'manifold pressure sensor fault'],
  [['intake manifold', 'air intake', 'manifold', 'air dryer'], 'engine air intake temperature high'],
  // Exhaust and aftertreatment
  [['dpf', 'soot level', 'regeneration', 'ash level'], 'dpf high soot load'],
  [['egr'], 'egr valve fault'],
  [['egt', 'exhaust temp', 'pyrometer', 'back pressure', 'catalyst temp', 'exhaust brake', 'exhaust', 'scr', 'def', 'nox sensor', 'dosing module', 'aftertreatment', 'oxidation'], 'exhaust temperature high'],
  // Speed, position and governing
  [['overspeed', 'speed shutdown'], 'engine overspeed shutdown'],
  [['underspeed', 'underfreq'], 'engine underspeed'],
  [['crankshaft', 'camshaft', 'position sensor', 'speed sensor', 'engine timing', 'tdc error', 'timing correlation', 'rotation'], 'crankshaft position sensor fault'],
  [['governor', 'throttle', 'droop', 'isochronous', 'speed control', 'speed trim', 'actuator'], 'governor failure'],
  [['engine speed', 'rpm', 'speed'], 'engine underspeed'],
  // Starting
  [['glow plug', 'preheat', 'block heater'], 'engine preheating relay failure'],
  [['starter', 'cranking', 'fail to start', 'starting', 'air start', 'battery start', 'ignition circuit'], 'starter relay failure'],
  // Electrical and charging
  [['overvoltage', 'overexcitation', 'voltage regulator', 'avr', 'excitation', 'exciter', 'field'], 'alternator over voltage'],
  [['battery charger', 'charging system', 'charging', 'charge', 'battery', 'dc supply', 'undervoltage', 'underexcited', 'brownout', 'sag'], 'alternator under voltage'],
  [['vt failure', 'ct failure', 'sensing', 'metering', 'measurement', 'vt', 'ct', 'pt'], 'alternator voltage sensor failure'],
  [['breaker', 'mccb', 'overcurrent', 'short circuit', 'overload', 'trip', 'lockout', 'earth fault', 'ground fault', 'earth leakage', 'differential', 'negative sequence', 'reverse power', 'motoring', 'loss of field', 'stator earth'], 'alternator circuit breaker trip'],
  // Control, comms and HMI
  [['ecm power', 'power supply', 'supply voltage', 'power wire'], 'ecm power supply fault'],
  [['ecm internal', 'eeprom', 'memory', 'flash', 'cpu', 'processor', 'watchdog', 'firmware', 'software', 'rtc', 'clock', 'calibration', 'self-test', 'hardware'], 'ecm internal memory error'],
  [['can', 'j1939', 'modbus', 'rs485', 'rs232', 'ethernet', 'tcp', 'profibus', 'devicenet', 'canopen', 'dnp3', 'iec 61850', 'goose', 'mms', 'snmp', 'sntp', 'gateway', 'communication', 'network', 'data link', 'protocol', 'baud rate', 'parity error', 'termination', 'keep alive', 'telemetry', 'scada', 'rtu', 'slave', 'master'], 'can communication loss'],
  [['display', 'lcd', 'hmi', 'led', 'keypad'], 'control panel display failure'],
  [['transfer switch', 'ats', 'changeover', 'retransfer'], 'ats communication error'],
  [['accelerator', 'hand throttle', 'setpoint range', 'analog input', 'analog'], 'accelerator pedal cable break or short circuit'],
  // Module-to-module interfaces are bus problems in practice, so they take the
  // communications content rather than being left without any.
  [['ecu interface', 'ecm communication', 'engine ecu', 'expansion', 'plc interface', 'plc', 'remote', 'gprs', 'gsm', 'bluetooth', 'usb', 'serial', 'signal'], 'can communication loss'],
  // Generator-end electrical items route to output protection, which is where a
  // technician starts on any alternator-side complaint.
  [['stator', 'rotor', 'winding', 'insulation', 'alternator', 'generator protect', 'generator diff', 'phase loss', 'open phase', 'phase sequence', 'phase order', 'unbalance'], 'alternator circuit breaker trip'],
];

/**
 * Resolve subsystem-level diagnostic content for a template entry from its
 * category and subcategory. Returns null where no confident subsystem match
 * exists — a template entry with no match keeps its own content rather than
 * being given guidance for a subsystem it may not belong to.
 */
export function getKnowledgeBySubsystem(category: string, subcategory: string): FaultKnowledge | null {
  const hay = `${subcategory || ''} ${category || ''}`.toLowerCase();
  if (!hay.trim()) return null;
  for (const [keywords, target] of SUBSYSTEM_ROUTES) {
    for (const kw of keywords) {
      if (hay.includes(kw)) {
        const k = INDEX.get(normaliseFault(target));
        if (k) return k;
      }
    }
  }
