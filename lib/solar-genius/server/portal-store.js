...
function ensureDir() { try { if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true }); } catch { /* readonly fs */ } }
function loadJson() {
  ensureDir();
  try {
...
function saveJson(state) {
  // Serverless filesystems (Vercel /var/task) are read-only. Wrap the
  // atomic write so an EROFS / ENOENT does not crash the request — the
  // in-memory mirror still serves reads for the lifetime of the lambda.
  try {
    ensureDir();
    const tmp = STORE + '.tmp';
    ...
    // Persistence unavailable — keep going with in-memory state only.
  }
}
