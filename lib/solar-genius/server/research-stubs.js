...
const REGISTRY = {
  ...
  'computer-vision-bom': {
    ...
    implemented: false,
    requires: 'tesseract.js native worker + persistent runtime. Excluded from this serverless deployment because the WASM/worker assets exceed the function bundle limit. Reference implementation lives in components/solar-modules/crc/server/research-impl.js for self-hosted worker.',
    free_tools: ['tesseract.js (Apache-2.0) — when run on a worker host'],
    free_alternative: 'Manual BOM entry via the equipment-library UI; or self-host the CRC worker and proxy /api/research/cv-bom/run to it.'
  },
  ...
  'iot-direct-integration': {
    ...
    implemented: false,
    requires: 'mqtt.js with a long-lived TCP/WebSocket subscription. Incompatible with serverless function lifecycles (max ~10s execution, no shared state across cold starts). Reference implementation lives in components/solar-modules/crc/server/research-impl.js for a Node worker host.',
    free_tools: ['mqtt.js (MIT) — when run on a persistent Node host'],
    free_alternative: 'Self-host the CRC worker (Express + ioredis ring buffer) and proxy /api/research/iot-mqtt/* to it; or push inverter telemetry into a webhook endpoint and read it via /api/research/ai-fault-prediction/run.'
  },
  'ar-roof-overlay': {
