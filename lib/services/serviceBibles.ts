...
const GENERATOR_BIBLE: ServiceBible = {
  ...
  intro: [
    ...
    'Repair economics deserve to be stated honestly, because the sector has a reputation for the opposite. A well-maintained diesel genset is a long-lived asset — the engine is designed for tens of thousands of hours — so a unit failing early is usually telling you about installation, fuel or maintenance rather than about its own design life. Before quoting an overhaul we want three things: the running hours, the maintenance history, and a load-bank result showing what the set can actually deliver today. Those three numbers separate a set that needs an injector service from one that genuinely needs a rebuild, and they occasionally show that neither is worth doing. We would rather tell a client their twenty-year-old set has reached the end than sell them a rebuild that will not pay for itself, and any quote that arrives without that evidence base is a guess dressed up as a diagnosis.',
  ],
  topBrands: [
  ...
  references: [
    ...
    'ISO 8528-5 — generating sets, engine performance and load acceptance',
...
const DISTRIBUTION_BIBLE: ServiceBible = {
  ...
  repairManual: [
    ...
    { fault: 'Single-phasing on a three-phase board', priority: 'emergency', steps: [
      ...
      'Compute voltage unbalance; even 2% unbalance produces roughly 6–10× that figure in negative-sequence current heating in connected motors.',
  ...
  errorCodes: [
    ...
    { code: '46 — negative sequence', family: 'ANSI/IEEE device function', meaning: 'Phase unbalance or reversed phase sequence detected; motors overheat on negative-sequence current.', severity: 'high', fix: ['Measure all three phase voltages and compute % unbalance', 'Check for a blown fuse or open pole upstream', 'Rebalance single-phase loads across the three phases', 'Confirm phase rotation after any switchgear work'] },
...
const SOLAR_BIBLE: ServiceBible = {
  ...
  intro: [
    ...
    'The financial model decides the design, not the other way round. Kenya\'s Energy Act 2019 provides for net metering on systems up to 1 MW, but the practical rollout has been slow and the terms depend on your supply agreement — so we size for self-consumption first and treat any export credit as upside. That single assumption changes the answer: a self-consumption design puts the array where the daytime load is, keeps the DC/AC ratio near 1.2, and only adds storage where the evening load or outage exposure justifies it. Systems sized on annual kWh instead of the hourly load curve routinely generate power at noon that nobody on site is using.',
...
const MOTOR_BIBLE: ServiceBible = {
  ...
  partsManual: [
    ...
    { group: 'Rotor, shaft & drive-end', items: [
      ...
      { name: 'Balancing weights and correction discs', note: 'Correct to ISO 1940 G2.5; record the residual unbalance on the test sheet.' },
  ...
  qualityChecks: [
    ...
    'No-load current balanced across phases within 5%',
    'Rotor balanced to ISO 1940 G2.5 with residual unbalance recorded',
...
const PUMP_BIBLE: ServiceBible = {
  ...
  references: [
    ...
    'ISO 17769 — liquid pumps, terms and definitions for pumping systems',
    'BS EN 12723 — liquid pumps, general terms for pumps and installations',
  ],
};
...
const INCINERATOR_BIBLE: ServiceBible = {
  ...
  references: [
    ...
    'WHO / UNICEF guidance on health-care waste segregation and colour coding',
    'Basel Convention technical guidelines on biomedical and healthcare waste',
    'BS EN 12952 / EN 746 — industrial thermoprocessing equipment safety',
  ],
};
