...
**Key facts:** WhatsApp/Facebook/most messaging apps **strip EXIF GPS permanently** when a photo is sent "as photo". Deleted metadata cannot be recovered by any software — Google included (Google Lens does *landmark recognition*, not metadata recovery, and only works for well-photographed landmarks). Generic filename hints ("church compound") are correctly rejected. At Grade D/F the report labels coordinates "AI ESTIMATE — NOT measured", maps print "NO GPS — LOCATION NOT MEASURED", and the report downgrades to Regional Pre-Screening. *Customer guidance (now printed on the report cover): type the location manually, or send the photo **as a File/Document** (WhatsApp: attach → Document) or by email — that preserves GPS; sending "as photo" deletes it.*

**Administrative hierarchy resolver (added 2026-07-09):** whenever coordinates exist, `reverseGeocode` merges Nominatim street-level detail with BigDataCloud's explicit OSM admin ladder (level 2 country → 4 county → 6 constituency/sub-county → 8–10 ward), and the Site Identity section prints the full **Village → Ward → Town → Constituency → County → Country** table. **Live-verified for Kenya:** point (-0.9, 37.19) resolves Makuyu village → Makuyu ward → Maragua constituency → Murang'a county → Kenya (KE). Kenya quirk handled: Nominatim's flat `county` field often holds the *sub-county* ("Murang`a South"); the explicit level-4 name wins and the sub-county slides into the constituency slot. Hierarchies derived from estimated coordinates are labelled "region of the estimate, not the photo's true origin."
