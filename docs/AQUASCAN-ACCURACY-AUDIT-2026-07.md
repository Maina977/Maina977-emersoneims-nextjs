...
### DOCUMENTED — still open (not yet fixed; each needs a design decision)

| # | Severity | Defect | Where |
|---|---|---|---|
| 8 | HIGH | **Economics contradict wildly between pages:** payback 0.9 yrs (p3) vs 68 months (p19) vs 21 yrs kiosk (p20) vs 3 months (p35) vs 1.3 yrs (p48); total cost $12,320 (p3) vs $14,872 (p19) vs $8,680 (p69) vs $14,400–33,552 (p89); drilling $6,800 vs $2,420 vs $2,795 vs $6,438. The "Investor Summary" (p3) uses revenue that implies ~24 h/day pumping while p19 assumes 6 h solar. **Recommendation: one canonical economics engine; every other section must cite it, not recompute.** | `reportGenerator.ts` investor summary vs `Cost & ROI` vs `riskDecisionEngine` vs `drillingSuccessPredictor` |
| 9 | HIGH | **Success probability appears as 57.3%, 57%, 48% ("RELOCATE"), 47.8%, 40%, 34%, 67.3%, 86%, 92%, and a Monte Carlo CI of 0.6–99.2%** across sections. The exec summary says "USE THIS PROBABILITY" but three different *decisions* are printed: "INVESTIGATE FURTHER" (p2), "RELOCATE — consider alternative site" (p35), "DRILL WITH MONITORING" (p54). One report must have one verdict. | multiple engines, each printing its own verdict |
| 10 | MED | Water-table depth stated as 30 m, 13 m, 13.2 m, 15.4 m, 4.4 m and 44 m in different sections. Same single-source-of-truth problem. | multiple |
| 11 | MED | ERT section derives **11.68 m³/hr "EXCELLENT" yield from an aquifer whose own table says thickness = 0.00 m** (engine even prints "PHYSICAL IMPOSSIBILITY" yet keeps the yield). Sanitizer should zero the yield when thickness < 0.5 m. | `ertIntelligenceEngine.ts` / `sanitizeOutputs.ts` |
| 12 | MED | Elevation 450 m for the Central Highlands estimate (actual Murang'a area ≈ 1,400–2,500 m) — the DEM was queried at the fallback/centroid coordinates and then used for derating and climate. Consequence of the Grade-D location, but the report should suppress elevation-dependent conclusions at grade D/F. | pipeline gating |
| 13 | MED | Rainfall printed as 700 mm/yr (p21), 800 mm/yr (p24) and 1,400 mm/yr (p33/45) in one report — API fallback vs live values mixed. | water-budget vs recharge modules |
| 14 | LOW | Setback table shows "Min 50 / Actual 50 → NO (30m < 50m)" — actual-distance column echoes the minimum instead of the assumed distance. | `reportGenerator.ts` setback table |
| 15 | LOW | Drill-site map header prints `0.00000°, 0.00000°` (null island) when location is estimated. Should print "NO GPS" instead of zeros. | `reportMapGenerator.ts` |
