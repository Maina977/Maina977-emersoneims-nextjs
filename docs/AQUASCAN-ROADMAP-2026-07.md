...
### B3. Registry wells drawn on the site maps (visual credibility)
Plot the 22,820 bundled wells + WPDx hits as dots (green=functional/red=failed) on the drill-site and water-table maps. A customer seeing 14 real wells around their plot trusts the analysis instantly. *Effort: small (canvas layer in reportMapGenerator).*
...
### B7. Seasonal drilling-window callout
`historicalData.bestDrillingSeason` is already computed but buried — surface it in the exec summary ("drill Aug–Oct after long rains recharge"). *Effort: trivial.*

### B8. Regression guard in reportAuditor
Encode the 13 fixed integrity rules (no COMPLETED without field data, one economics model, one verdict, no synthetic wells…) as automated pre-export assertions so they can never regress. *Effort: small, high protection.*
