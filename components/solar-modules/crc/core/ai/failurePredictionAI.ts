...
class FailurePredictionAI {
  ...
  private async predictBatteryFailure(request: FailurePredictionRequest): Promise<ComponentPrediction> {
    ...
    const estimatedCycles = 500 + (()=>{throw new Error('DATA_POLICY: synthetic value refused — wire a real source. See DATA_POLICY.md')})() * 1000;
  ...
  private async predictPanelDegradation(request: FailurePredictionRequest): Promise<ComponentPrediction> {
    ...
    const ageYears = 2 + (()=>{throw new Error('DATA_POLICY: synthetic value refused — wire a real source. See DATA_POLICY.md')})() * 8;
    if (ageYears > 20) {
      failureProbability += 30;
      contributingFactors.push({
        name: 'Panel Age',
  ...
  private async predictWiringFailure(request: FailurePredictionRequest): Promise<ComponentPrediction> {
    ...
    const ageYears = 3 + (()=>{throw new Error('DATA_POLICY: synthetic value refused — wire a real source. See DATA_POLICY.md')})() * 10;
    if (ageYears > 15) {
      failureProbability += 25;
      contributingFactors.push({
  ...
  private async getTelemetry(systemId: string, component: string): Promise<ComponentTelemetry[]> {
    ...
    return Array(30).fill(null).map((_, i) => ({
      ...
      temperature: 45 + (()=>{throw new Error('DATA_POLICY: synthetic value refused — wire a real source. See DATA_POLICY.md')})() * 15,
      voltage: 380 + ((()=>{throw new Error('DATA_POLICY: synthetic value refused — wire a real source. See DATA_POLICY.md')})() - 0.5) * 20,
      current: 15 + (()=>{throw new Error('DATA_POLICY: synthetic value refused — wire a real source. See DATA_POLICY.md')})() * 10,
      power: 5000 + (()=>{throw new Error('DATA_POLICY: synthetic value refused — wire a real source. See DATA_POLICY.md')})() * 2000,
      errorCount: (()=>{throw new Error('DATA_POLICY: synthetic value refused — wire a real source. See DATA_POLICY.md')})() > 0.9 ? 1 : 0,
  ...
  async getFailureProbabilityTrend(systemId: string, days: number = 30): Promise<{
    ...
    for (let i = days; i >= 0; i--) {
      ...
      dates.push(date);
      probabilities.push(5 + Math.sin(i / 10) * 10 + (()=>{throw new Error('DATA_POLICY: synthetic value refused — wire a real source. See DATA_POLICY.md')})() * 5);
    }
